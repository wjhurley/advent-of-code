...
const findLineIntersections = (lineA: number[][], lineB: number[][]): number[][] => {
    ...
    for (let i = 0; i < lineA.length - 1; i++) {
        ...
        for (let j = 0; j < lineB.length - 1; j++) {
            ...
            if (
                minAX === maxAX && minBY === maxBY // Check for no change on lineA x-axis or lineB y-axis (lines are perpendicular)
                ...
                minAY === maxAY && minBX === maxBX // Check for no change on lineA y-axis or lineB x-axis
...
const findLineIntersectionSteps = (intersection: number[], lineA: number[][], lineB: number[][]): number => {
    const findSteps = (line: number[][]): number => {
        ...
        while (!didLineHitIntersection && i < line.length - 1) {
            ...
            if (
                minX === maxX && minX === intersection[0] // Check if line and intersection share the same x-axis
                ...
                minY === maxY && minY === intersection[1] // Check if line and intersection share the same y-axis
